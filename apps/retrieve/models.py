...
class SearchFilter(models.Model):
    ...
    def search_filterPara(self):
        ...
        text = '{}:"{}"{}"{}",{}:{},{}:{},{}:{},{}:{}'.format(
            # 题目或其他搜索条件
            SEARCH_DIC.get(search_filter_keys[0] or 'txt_2_sel', dict()).get(search_filter.get(search_filter_keys[0], ''), 'SU'),
            search_filter.get(search_filter_keys[1], ''),
            SEARCH_DIC.get(search_filter_keys[2] or 'txt_2_relation1', dict()).get(search_filter.get(search_filter_keys[2], '')),
            search_filter.get(search_filter_keys[3], ''),
            # 作者
            SEARCH_DIC.get(search_filter_keys[4] or 'au_1_sel', dict()).get(search_filter.get(search_filter_keys[4], 'AU'), ''),
            search_filter.get(search_filter_keys[5], ''),
            # 机构组织
            SEARCH_DIC.get(search_filter_keys[6] or 'org_1_sel', dict()).get(search_filter.get(search_filter_keys[6], 'ORG'), ''),
            search_filter.get(search_filter_keys[7], ''),
            # 文献号或名字
            SEARCH_DIC.get(search_filter_keys[8] or 'magazine_1_sel', dict()).get(search_filter.get(search_filter_keys[8]), 'issn_number'),
