...
class CrawlCnkiSummary(object):
    ...
    def incremental_crawl(self, *args, start_num=0, mark=False, issn_number=0):
        ...
        for periodical in periodicals:
            ...
            self.get_incremental_periodicals_summary(periodical)

    def get_incremental_periodicals_summary(self, periodical):
        ...
        driver.get(start_url)
        elem = driver.find_element_by_id("magazine_value1")  # 找到name为magazine_value1的元素，这里是个搜索框
        elem.send_keys(periodical.issn_number)
        select = Select(driver.find_element_by_id('magazine_special1'))  # 通过Select来定义该元素是下拉框
        select.select_by_index(1)  # 通过下拉元素的位置来选择，选择为"精准"
        select = Select(driver.find_element_by_id('updatedateN_opt'))  # 通过Select来定义该元素是下拉框
        select.select_by_index(1)  # 通过下拉元素的位置来选择，选择为"最近一周"
        elem.send_keys(Keys.RETURN)  # 相当于回车键，提交
        time.sleep(2)
        driver.get(
            url=self._root_url + '/kns/brief/brief.aspx?curpage=1&RecordsPerPage=20&QueryID=20&ID=&turnpage=1&dbPrefix=CJFQ&PageName=ASP.brief_result_aspx#J_ORDER&'
        ...
        for num in range(1, pagenums + 1):
            print(periodical.issn_number + ":page-" + str(num))
            # 遍历每一页
            driver.get(
